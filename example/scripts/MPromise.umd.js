(function (global, factory) {
	...
	(global = global || self, factory(global.MPromise = {}));
	...
	class MPromise {
	    ...
	    constructor(revealing) {
	        ...
	        if (revealing) {
	            ...
	            const reject_ = val => {
	                ...
	                called = true;
	                this[setValueAndStatus](val, 2 /* rejected */);
	            };
	            try {
	                ...
	                if (!called) {
	                    this[setValueAndStatus](err, 2 /* rejected */);
	                }
	            }
	        }
	    }
	    ...
	    // 类型, 所以也是any
	    static [_resolve](promise, x) {
	        // 虽然setValueAndStatus中实际上对状态进行了控制, 不过这里还是判断一下
	        // 避免意外
	        if (promise[_status] !== 0 /* pending */) {
	            return;
	        }
	        // 2.3.1. 如果 `promise` 和 `x` 指向同一个对象, 则将 `promise` 置为 rejected, 并用一个 `TypeError` 作为 reason
	        if (promise === x) {
	            promise[setValueAndStatus](new TypeError('promise is same as x'), 2 /* rejected */);
	            ...
	            try {
	                ...
	                promise[setValueAndStatus](err, 2 /* rejected */);
	                return;
	            }
	            ...
	            if (typeof then === 'function') {
	                ...
	                const resolvePromise = y => {
	                    ...
	                    // 3.6 检测循环thenalbe
	                    if (x === y) {
	                        promise[setValueAndStatus](new TypeError('cycle thenable'), 2 /* rejected */);
	                        return;
	                    }
	                ...
	                const rejectPromise = r => {
	                    ...
	                    promise[setValueAndStatus](r, 2 /* rejected */);
	                };
	                try {
	                ...
	                promise[setValueAndStatus](x, 1 /* fulfilled */);
	            }
	        }
	        else {
	    ...
	    static reject(x) {
	        if (x instanceof MPromise) {
	            ...
	            return new MPromise((rs, rj) => rj(x));
	        }
	    }
	    // 状态和value总是一起改变的, 所以通过统一的私有方法进行设置便于管理
	    [setValueAndStatus](value, status) {
	        // 2.1 的约束
	...
	// 为了跑测试暴露出来
	const resolve = MPromise[_resolve];
	// 为了跑测试暴露出来
	function reject(promise, reason) {
	    promise[setValueAndStatus](reason, 2 /* rejected */);
	}

	exports.MPromise = MPromise;
...
//# sourceMappingURL=MPromise.umd.js.map
